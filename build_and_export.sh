...
echo "[1/3] 开始构建 lvs-router-lb 镜像（lb1 / lb2）..."
docker build --platform=linux/amd64 --build-arg LB=1 -t lvs-router-lb:lb1 ./lb
docker build --platform=linux/amd64 --build-arg LB=2 -t lvs-router-lb:lb2 ./lb
...
docker save -o lvs-offline-images.tar lvs-router-lb:lb1 lvs-router-lb:lb2 curlimages/curl:8.8.0

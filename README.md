...
- 主备倒换时，除了 VIP 漂移，还需要额外启动或停止一次独立的 IPVS/TCP_CHECK Keepalived 实例
- 恢复时间通常会比“双机都常驻 `virtual_server` / IPVS 规则”的方案略慢
- 如果切换后还需要等待首轮 `TCP_CHECK` 把可用 RS 加回池中，切换窗口会更明显

如果你的环境对切换收敛时间非常敏感，需要在“避免 LB/RS 重合带来的递归转发风险”和“尽量缩短故障切换恢复时间”之间做权衡。

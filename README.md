...
如果发生 VRRP 双主，且两台机器同时处于 `LB/RS merged` 拓扑，后果会比“纯 LB 双主”更严重。原因在于这两台机器不仅都可能持有同一个 VIP，还都同时兼任 `real_server`，并且都可能保留 director 身份。

典型影响包括：

- VIP 双持有：两台机器都可能绑定同一个 VIP，并对外发送 GARP，导致客户端或交换机上的 ARP 表来回抖动
- 流量路径不稳定：同一个客户端的连续请求可能落到不同 LB，上层表现为时好时坏、偶发失败或难以复现
- IPVS 规则双活：两边都运行 IPVS/TCP_CHECK，并同时维护相同的 `virtual_server` / `real_server` 规则
- 递归转发风险放大：一台 LB 第一次调度后的流量，如果进入另一台同样保留 IPVS 规则的 LB/RS 节点，可能再次触发本机 LVS 处理，形成二次调度甚至异常回环
- 请求间歇性失败：并不一定表现为全部不可用，更常见的是部分请求成功、部分超时、部分命中错误后端，或者按固定节奏失败
- 连接状态混乱：长连接、会话保持和连接模板可能同时受到旧路径、新路径和双主调度的影响
- 健康检查视图分裂：两边都在运行 `TCP_CHECK`，但各自看到的后端健康状态可能不完全一致，从而基于不同视图做调度
- 回切后仍可能残留异常：即使后来恢复成单主，ARP 缓存、旧连接和 IPVS 状态也可能让问题延续一段时间

因此，`LB/RS merged` 场景下最怕的不是“两个 LB 同时服务”本身，而是“两台既是 LB 又是 RS 的节点，同时保留 director 身份”。这会把 VIP 冲突、规则双活和递归转发叠加在一起，排障成本也会显著上升。
